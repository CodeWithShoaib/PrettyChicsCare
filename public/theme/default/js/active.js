 (function($) {
	...
	$(document).on('change','.customer-country',function(){
		...
		if(country != ""){
			...
			var _url='http://localhost/newProject';
			$.ajax({
				...
				beforeSend: function(){
			    	...
			    	$('#state').html('<option value="" style="color:#000;">' + $("#state option:first").text() + '</option>');
			    	$.each(json, function(i, item) {
					    $('#state').append(`<option style="color:#000;" value="${item.name}">${item.name}</option>`);
					});
